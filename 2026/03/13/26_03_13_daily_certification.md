...
public class Main {
  public static void main(String[] args) throws IOException {
    ...
    parametricSearch(n, m, beads, min, max);
  }	// main-end

  private static void parametricSearch(int n, int m, int[] beads, int min, int max) {
    int start = min;
    int end = max;
    int res = 0;
    ...
    while(start <= end) {
      int mid = (start + end) / 2;
      int cnt = cnt(n, beads, mid);	//	구간합의 허용 최댓값을 mid로 했을때 구간의 최소 개수
  ...
  private static int cnt(int n, int[] beads, int maxPs) {
    int res = 1;
    int ps = 0;
