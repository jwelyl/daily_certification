...
[31963번: 두 배](http://boj.ma/31963/t)
...
public class Main {
  public static void main(String[] args) throws IOException {
    ...
    long[] arr = new long[n];
    //  cntArr[i] : 비내림차순 유지를 위해 i번째 수에 2를 곱한 횟수
    ...
    for(int i = 1; i < n; i++) {
      double idx = Math.ceil(Math.log((double)arr[i - 1] / arr[i]) / Math.log(2)) + cntArr[i - 1];
